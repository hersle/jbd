...
if len(sys.argv) < 4:
  print("Error: missing arguments [dictfile bashfileprefix nnodes]")
  exit(1)
dictfile       = sys.argv[1]
...
class Generate:
  ...
  def __init__(self, run_param, bashfiles = None, debug = False):
    ...
    self.N_ncdm     = 1#int(Omega_ncdm>0)          # Massive neutrino species
    ...
    if self.cosmology == "JBD" or self.gravity == "JBD":
      self.wBD = 10**cosmo_param['log10wBD']
      if not self.use_physical_parameters:
        print("Error JBD have to be run with physical parameters")
    else:
      self.wBD = 0.0
    if self.gravity == "f(R)":
      ...
      self.fofr0 = 0.0
    if self.gravity == "Geff":
      self.mu0 = cosmo_param['mu0']
    else:
      self.mu0 = 0.0
  ...
  # Define the mu = Geff/G(a) function. This simple example is
  # Geff/G = 1 + mu0 * OmegaLambda(a) (for w=-1)
  def GeffoverG(self,a):
    Omega_m = 1.0 - self.Omega_fld
    return 1.0 + self.mu0 * self.Omega_fld/(Omega_m/a**3 + self.Omega_fld)
  ...
  def generate_data(self, GR):
    ...
    # [1] Make the Geff(a) file needed inside COLA
    if gravity == "Geff":
      gefffile = self.outputfolder + "/geff_of_a_"+self.name+".txt"
      aarr = np.linspace(0.01,1.0,1000)
      GeffoverGarr = self.GeffoverG(aarr)
      np.savetxt(gefffile, np.c_[aarr, GeffoverGarr])
    else:
      gefffile = ""

    # [2] Make list of redshifts 
    ...
    # [3] Set class parameters and run class
    params = {
        'root': 'output/class_'+self.name+'_',
        'format': 'camb',
        'output': 'tCl,mPk,mTk',
        'l_max_scalars': 2000,
    ...
    # [4] Extract and output transfer information (in CAMB format suitable for COLA)
    ...
    # [5] Write transferinfo-file needed by COLA
...
if cosmology_model == \"JBD\" then                           \n\
  cosmology_JBD_wBD = "+str(self.wBD)+"                      \n\
  cosmology_JBD_GeffG_today = 1.0                            \n\
  -- We require physical parameters. h is a derived quantity \n\
  cosmology_JBD_Omegabh2 = "+str(self.omega_b)+"             \n\
  cosmology_JBD_OmegaMNuh2 = "+str(self.omega_ncdm)+"        \n\
  cosmology_JBD_OmegaCDMh2 = "+str(self.omega_cdm)+"         \n\
  cosmology_JBD_OmegaLambdah2 = "+str(self.omega_fld)+"      \n\
  cosmology_JBD_OmegaKh2 = "+str(self.omega_k)+"             \n\
end                                                          \n\
                                                             \n\
------------------------------------------------------------ \n\
...
if gravity_model == \"Geff\" then                            \n\
  ...
  gravity_model_geff_geffofa_filename = \""+gefffile+"\"     \n\
end                                                          \n\
                                                             \n\
...
bispectrum_density_assignment_method = \"PCS\"               \n\
  """
    ...
    # [6] Compute HMCode pofk from class (useful to have)
